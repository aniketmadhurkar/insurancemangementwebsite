...
if (isset($_POST['thisform'])) {
    ...
    $single_sql = "SELECT customer_id from customers WHERE customer_id=$customer_id  and password='$old_password'";
    $single_result = mysqli_query($con, $single_sql);
    $single_row = mysqli_fetch_assoc($single_result);
    if (mysqli_num_rows($single_result) > 0) {

        $sql = "update customers  set password ='$new_password' where customer_id=$customer_id and password='$old_password'";
